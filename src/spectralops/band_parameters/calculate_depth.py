# band_parameters/calculate_depth.py
...
def calculate_depth_from_spectrum(
    wvl: np.ndarray,
    ...
    """
    Calculate the absorption depth from a continuum-removed spectrum.
    ...
    return calculate_depth_from_fit(
        fitted_absorption,
        absorption_spec,
        absorption_wvl
    )
...
def calculate_depth_from_fit(
    fitted_absorption: np.ndarray,
    absorption_spec: np.ndarray,
    absorption_wvl: np.ndarray
) -> Union[float, np.ndarray]:
    """
    Calculate the absorption depth from a fitted absorption feature.
    ...
    if fitted_absorption.ndim == 3:
        ...
        absorption_center_spec_val = fitted_absorption[
            *np.indices(absorption_center.shape), absorption_center
        ]
        absorption_center_spec_val[np.isnan(absorption_center_wvl)] = np.nan

        return 1 - absorption_center_spec_val
    else:
        ...
        if (absorption_center == 0) or \
           (absorption_center == absorption_spec.size):
            ...
            return 1 - fitted_absorption[absorption_center]
...
def calculate_depth(
    ...
    if use_spectrum_mode:
        ...
        return calculate_depth_from_spectrum(
            wvl,
